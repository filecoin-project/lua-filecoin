...
for _, test in ipairs(tests) do
  print()
  p(test)
  local encoded, decoded, name, length, multi, hash
  encoded, name, length = Multihash.encode(test.input, test.name, test.givenLength)
  multi = hex.encode(encoded)
  p(multi)
  assert(multi == test.multi, 'multihash encoding failure')
  assert(name == test.name, 'encoding name mismatch')
  assert(length == test.length, 'encoding length mismatch')
  decoded, name, length = Multihash.decode(encoded)
